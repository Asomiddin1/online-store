...
const Navigator = () => {
  ...
  const showMenu =(e)=>{
    e.stopPropagation()
    if (e.target.className.includes('munna')) {
    ...
    <div className='flex w-full justify-around justify-items-center py-3 bg-[#fff] border-b-2 border-b-[#717181]' >
      ...
      {menu && <div onClick={showMenu} className='munna fixed top-0 left-0 flex justify-center items-center right-0 bottom-0 bg-[#000000c4] z-50 backdrop-blur'>
         <div className='left-0 fixed  top-0 bottom-0 w-[300px] bg-white  pl-6'>
          <div className='px-[10px] py-6 flex justify-between'>
             <FontAwesomeIcon onClick={()=> setMenu(false)} className='munna fa-2x'  icon={faX}/>
